...
libFuzzer 将整个被测对象视为一个函数接口，每次测试视为循环体中对函数的一次调用。但是 AFL 不然，他将被测对象看作一个可执行程序，每次测试视为对程序的一次调用，自然也会在新的进程环境中。
...
元组（tuple）：具体到 AFL 的实现中，使用二元组(branch_src, branch_dst)来记录当前基本块 + 前一基本块 的信息，从而获取目标的执行流程和代码覆盖情况
